    ...
    useEffect(() => {
        ...
        if (liveClass) {
            ...
            const fetchInitial = async () => {
                try {
                    ...
                    if (res.ok) {
                        ...
                        connectWS();
                        // Start database-backed trend polling
                        startTrendPolling();
                    }
                } catch (e) { console.error(e); }
            };
            ...
            const startTrendPolling = () => {
                // Poll backend trend endpoint every 10 seconds
                const fetchTrend = async () => {
                    try {
                        const res = await fetch(`http://localhost:8000/classroom/${liveClass.id}/engagement_trend?minutes=30`);
                        if (!res.ok) return;
                        const data = await res.json();

                        if (!Array.isArray(data)) return;

                        // Group by timestamp bucket (minute) and compute class average
                        const buckets: Record<string, { total: number; count: number }> = {};
                        data.forEach((item: any) => {
                            if (typeof item.engagement_score !== 'number') return;
                            if (!item.timestamp) return;
                            const minuteKey = item.timestamp.slice(0, 16); // "YYYY-MM-DDTHH:MM"
                            if (!buckets[minuteKey]) {
                                buckets[minuteKey] = { total: 0, count: 0 };
                            }
                            buckets[minuteKey].total += item.engagement_score;
                            buckets[minuteKey].count += 1;
                        });

                        const sortedKeys = Object.keys(buckets).sort();
                        const series = sortedKeys.map(key =>
                            Math.round(buckets[key].total / Math.max(1, buckets[key].count))
                        );

                        // Keep last 50 points to match existing behavior
                        setEngagementHistory(prev => {
                            const merged = [...prev, ...series];
                            const unique = merged.slice(-50);
                            return unique;
                        });
                    } catch (err) {
                ...
                // Immediately fetch once, then set interval
                fetchTrend();
                trendInterval = window.setInterval(fetchTrend, 10000);
            };
                        ...
                        >
                            <div>
                                ...
                                <div className="flex flex-wrap items-center gap-2 mt-2">
                                    <span className="px-3 py-1 rounded-full bg-slate-700/50 border border-slate-600 text-slate-300 font-mono text-xs tracking-wider">
                                        ID: {cls.id}
                                    </span>
                                    <span className="px-3 py-1 rounded-full bg-slate-700/50 border border-slate-600 text-slate-300 font-mono text-xs tracking-wider">
                                        Code: {cls.code}
